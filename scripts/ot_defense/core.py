...
TCP_RISKS: dict[int, ServiceRisk] = {
    ...
    23: ServiceRisk(
        ...
        "Telnet",
        "critical",
        "Telnet is cleartext remote administration. It should not face public networks.",
    ...
    20256: ServiceRisk(
        ...
        "critical",
        "Unitronics PLC programming exposure has been targeted in water-sector incidents.",
        "Remove internet exposure, rotate PLC passwords, patch, and verify logic backups.",
    ),
}
            ...
            if "/" in token:
                ...
                if network.num_addresses > max_hosts:
                    raise TargetParseError(
                        ...
                        f"max-hosts is {max_hosts}. Narrow the scope or raise the limit."
                    )
            ...
            if future.result():
                risk = TCP_RISKS.get(
                    ...
                    ServiceRisk(
                        ...
                        "Confirm business need and restrict exposure to approved paths.",
                    ),
                )
                findings.append(
                    ScanFinding(
...
def render_markdown(report: ScanReport) -> str:
    ...
    lines = [
        ...
        "- Preserve logs/configs and coordinate with CISA/FBI/EPA/WaterISAC if incident indicators exist.",
        "",
    ]
    ...
    if not report.findings:
        lines.extend(
            [
                ...
                "That does **not** prove the environment is secure; it only means these ports did not answer here.",
                "",
            ]
        )
        return "\n".join(lines)
    ...
    for finding in report.findings:
        lines.append(
            f"- `{finding.host}:{finding.port}` **{finding.service}**: {finding.rationale}"
        )
    lines.append("")
    return "\n".join(lines)
