...
- `approval-decision` — frames the only authoritative permission object in the local governance chain, and writes only the canonical `approval_decision` object while keeping future lease identity bound to the stable authority principal and shaping any needed `lease_request` object.
...
- `governance-orchestrator` — explicitly runs `fast-path check -> (if needed) handshake -> workflow-state -> execution-plan -> lease-request -> approval-decision -> workflow-commit -> lease-audit -> journal-audit` against the DroidPuppy context packet and can mint a lease from `approval_decision.lease_request`.
