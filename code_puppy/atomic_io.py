...
def _copy_exclusive(source: str, destination: str) -> None:
    """Durably copy to a new path on platforms without hard-link support."""
    mode = os.stat(source).st_mode
    fd = os.open(destination, os.O_WRONLY | os.O_CREAT | os.O_EXCL, mode & 0o777)
    try:
        with open(source, "rb") as source_file, os.fdopen(fd, "wb") as backup:
            fd = -1
            while chunk := source_file.read(1024):
                backup.write(chunk)
            backup.flush()
...
def _create_quarantine_backup(source: str, destination: str) -> None:
    hard_link = getattr(os, "link", None)
...
def quarantine_file(path: str) -> str:
    ...
    Prefer an atomic hard link. Android Python builds that omit ``os.link``
    use an exclusive, fsynced same-directory copy. In both cases the original
    is removed only after a durable, non-overwriting backup exists.
    """
    for _ in range(10):
        quarantine_path = f"{path}.corrupted-{time.time_ns()}-{uuid.uuid4().hex}"
        try:
            _create_quarantine_backup(path, quarantine_path)
