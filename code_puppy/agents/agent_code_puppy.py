...
class CodePuppyAgent(BaseAgent):
    ...
    def get_available_tools(self) -> list[str]:
        """Get the list of core tools available to Code-Puppy.

        Plugin-owned tools are intentionally not hardcoded here. They are
        advertised dynamically via the ``register_agent_tools`` callback so the
        agent surface matches the plugins actually loaded for the current
        runtime profile.
        """
        return [
            "list_agents",
