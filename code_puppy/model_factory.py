...
def _thinking_tags_profile(model_name: str, model_config: dict[str, Any]) -> Any | None:
...
def make_model_settings(
    ...
    if is_copilot and copilot_underlying.startswith("claude-"):
        ...
        # Copilot GPT/O-series does not support reasoning_effort.
        model_settings = _make_openai_chat_settings(model_settings_dict)
        ...
        model_settings_dict["openai_reasoning_effort"] = (
            normalize_openai_reasoning_effort(
                model_name,
                effective_settings.get("reasoning_effort", "medium"),
                model_config,
            )
        )
        ...
        requested_effort = effective_settings.get("reasoning_effort", "medium")
...
class ModelFactory:
    ...
    def get_model(model_name: str, config: Dict[str, Any]) -> Any:
        ...
        # via the register_model_type callback. See plugins/claude_code_oauth/register_callbacks.py
            ...
            # Cerebras rejects mixed strict tool values. Use the compatibility
            # loader so both pinned Android and newer upstream runtimes work.
