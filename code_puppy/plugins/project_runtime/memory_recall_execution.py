...
def _lease_blockers(lease: lease_store.LeaseRecord, now: datetime) -> tuple[str, ...]:
    blockers: list[str] = []
    ...
    if lease.action_scope != MEMORY_RECALL_ACTION_SCOPE:
        blockers.append("lease action scope mismatch")
    if lease.capability_scope != MEMORY_RECALL_CAPABILITY_SCOPE:
