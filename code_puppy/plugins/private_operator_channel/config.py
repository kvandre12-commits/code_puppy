...
DEFAULT_EFFECTFUL_TOOL_NAME = "android_open"
DEFAULT_EFFECTFUL_CAPABILITIES = {
    "android_open": {
        "settings": "android.settings.open",
        "browser": "android.browser.open_url",
        "launch": "android.app.launch",
    }
}
...
class PrivateOperatorChannelConfig:
    ...
    require_governance_for_effectful_actions: bool = True
    ...
    def with_overrides(self, **overrides: Any) -> "PrivateOperatorChannelConfig":
        ...
        if isinstance(data.get("allowed_actions"), list):
            data["allowed_actions"] = tuple(
                str(item) for item in data["allowed_actions"]
            )
    ...
    def effectful_actions(self) -> tuple[str, ...]:
        return tuple(
            action for action in self.allowed_actions if action in EFFECTFUL_ACTIONS
        )
...
def _normalize_loaded_mapping(raw: dict[str, Any]) -> dict[str, Any]:
    ...
    for key in (
        ...
        "require_governance_for_effectful_actions",
    ):
        if key in raw:
            normalized[key] = raw[key]
    if "allowed_actions" in raw and isinstance(raw["allowed_actions"], list):
        normalized["allowed_actions"] = tuple(
            str(item) for item in raw["allowed_actions"]
        )
    if "allowed_android_targets" in raw and isinstance(
        raw["allowed_android_targets"], list
    ):
...
def write_example_config(
    ...
    path.write_text(
        json.dumps(example_config_dict(), indent=2) + "\n", encoding="utf-8"
    )
    return {
        "success": True,
        "path": str(path),
...
def config_snapshot(
    ...
    *,
    ...
    return {
        ...
        "require_governance_for_effectful_actions": config.require_governance_for_effectful_actions,
    }
