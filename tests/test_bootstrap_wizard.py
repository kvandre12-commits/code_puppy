...
def test_bare_termux_device_builds_three_steps(monkeypatch):
    """A fresh phone with nothing installed gets uv -> packages -> code-puppy."""
    ...
    # Termux installs uv via pip, not the curl installer.
    assert by_key["uv"].command == "pip install uv"
    assert by_key["system_packages"].command == "pkg install -y ripgrep proot"
...
def test_yes_runs_steps_and_verifies(monkeypatch, capsys):
    _patch_env(
        monkeypatch, is_android=True, is_termux=True, has_ripgrep=False, has_proot=False
    )
    ...
    assert any("pkg install -y ripgrep proot" in c for c in calls)
