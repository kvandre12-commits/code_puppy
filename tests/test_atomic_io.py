...
class TestQuarantineFile:
    ...
    def test_copy_fallback_preserves_payload_when_hard_links_are_missing(
        self, target_path, monkeypatch
    ):
        target_path.write_bytes(b"android payload")
        monkeypatch.delattr(atomic_io.os, "link", raising=False)

        quarantine_path = atomic_io.quarantine_file(str(target_path))

        assert not target_path.exists()
        assert open(quarantine_path, "rb").read() == b"android payload"
